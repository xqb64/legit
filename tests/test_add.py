...
def test_it_adds_multiple_files_to_the_index(write_file, legit_cmd, repo):
    write_file("hello.txt", "hello")
...
def test_it_adds_a_directory_to_the_index(write_file, legit_cmd, repo):
    write_file("a-dir/nested.txt", "content")
    _ = legit_cmd("add", "a-dir")
    assert get_index(repo) == [(0o100644, "a-dir/nested.txt")]


def test_it_adds_the_repository_root_to_the_index(write_file, legit_cmd, repo):
    write_file("a/b/c/file.txt", "content")
    _ = legit_cmd("add", ".")
    assert get_index(repo) == [(0o100644, "a/b/c/file.txt")]


def test_it_is_silent_on_success(write_file, legit_cmd):
    write_file("hello.txt", "hello")
...
def test_it_fails_for_nonexistent_files(legit_cmd, repo):
...
def test_it_fails_for_unreadable_files(write_file, make_unreadable, legit_cmd, repo):
...
def test_it_fails_if_the_index_is_locked(write_file, legit_cmd, repo):
    write_file("file.txt", "")
