...
class TestConfig:
    def test_it_returns_1_for_unknown_variables(self, legit_cmd):
        cmd, *_ = legit_cmd("config", "--local", "no.such")
        assert_status(cmd, 1)

    def test_it_returns_1_when_key_is_invalid(self, legit_cmd):
        cmd, *_, stderr = legit_cmd("config", "--local", "0.0")
        assert_status(cmd, 1)
        assert_stderr(stderr, "error: invalid key: 0.0\n")

    def test_it_returns_2_when_no_section_is_given(self, legit_cmd):
    ...
    def test_it_returns_the_value_of_a_set_variable_in_subsection(self, legit_cmd):
    ...
    def test_it_unsets_a_variable(self, legit_cmd):
        _ = legit_cmd("config", "core.editor", "ed")
    ...
    class TestMultiValuedVariables:
        ...
        def test_it_replaces_a_variable(self, legit_cmd):
        ...
        def test_it_returns_5_on_trying_to_unset_a_variable(self, legit_cmd):
        ...
        def test_it_unsets_all_values_for_a_variable(self, legit_cmd):
    ...
    def test_it_removes_a_subsection(self, legit_cmd):
        legit_cmd("config", "core.editor", "ed")
        legit_cmd("config", "remote.origin.url", "ssh://example.com/repo")
