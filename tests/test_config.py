...
class TestConfig:
    def test_it_returns_1_for_unknown_variables(self, legit_cmd: LegitCmd) -> None:
        cmd, *_ = legit_cmd("config", "--local", "no.such")
        assert_status(cmd, 1)

    def test_it_returns_1_when_key_is_invalid(self, legit_cmd: LegitCmd) -> None:
        cmd, *_, stderr = legit_cmd("config", "--local", "0.0")
        assert_status(cmd, 1)
        assert_stderr(stderr, "error: invalid key: 0.0\n")

    def test_it_returns_2_when_no_section_is_given(self, legit_cmd: LegitCmd) -> None:
        cmd, *_, stderr = legit_cmd("config", "--local", "no")
        assert_status(cmd, 2)
        assert_stderr(stderr, "error: key does not contain a section: no\n")

    def test_it_returns_the_value_of_a_set_variable(self, legit_cmd: LegitCmd) -> None:
    ...
    def test_it_unsets_a_variable(self, legit_cmd: LegitCmd) -> None:
        _ = legit_cmd("config", "core.editor", "ed")
        _ = legit_cmd("config", "--unset", "core.editor")
    ...
    class TestMultiValuedVariables:
        ...
        def test_it_replaces_a_variable(self, legit_cmd: LegitCmd) -> None:
            _ = legit_cmd("config", "--replace-all", "remote.origin.fetch", "new-value")
        ...
        def test_it_unsets_all_values_for_a_variable(self, legit_cmd: LegitCmd) -> None:
            legit_cmd("config", "--unset-all", "remote.origin.fetch")

            cmd, _, _, _ = legit_cmd("config", "--get-all", "remote.origin.fetch")
            assert_status(cmd, 1)

    def test_it_removes_a_section(self, legit_cmd: LegitCmd) -> None:
    ...
    def test_it_removes_a_subsection(self, legit_cmd: LegitCmd) -> None:
        legit_cmd("config", "core.editor", "ed")
        legit_cmd("config", "remote.origin.url", "ssh://example.com/repo")
