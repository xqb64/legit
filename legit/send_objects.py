...
class SendObjectsMixin:
    def send_packet_objects(self, revs):
        ...
        pack_compression = self.repo.config.get(["pack", "compression"]) or self.repo.config.get(["core", "compression"]) or zlib.Z_DEFAULT_COMPRESSION

        write_opts = {"compression": pack_compression, "progress": Progress(self.stderr)}
