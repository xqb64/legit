...
class Push(FastForwardMixin, RemoteClientMixin, SendObjectsMixin, Base):
    ...
    def configure(self) -> None:
        ...
        self.receiver = self.options.get("receiver") or remote.receiver or self.RECEIVE_PACK
        self.push_specs = self.args[1:] if len(self.args) > 1 else remote.push_specs if remote is not None else None
    ...
    def select_deletion(self, target) -> None:
        if self.conn.capable("delete-refs"):
            ...
            self.errors.append([[None, target], "remote does not support deleting refs"])
    ...
    def send_objects(self) -> None:
        revs = [vals[-1] for vals in self.updates.values() if vals and vals[-1] is not None]
        if not revs:
            return
